#!/usr/bin/env node
/**
 * Compare trainMult values at different FARM_THRESHOLD settings
 * Using tierMult=20 (the winner from tier comparison)
 */
...
// =============================================================================
// AI RUNNER with configurable parameters
// =============================================================================

function runAIWithConfig(ai, clicks, enemyPower, tierMult, trainMult, farmThreshold) {
  const VAL_SL = 5;
  ...
  const VAL_FARM = VAL_SL * (farmThreshold / Math.max(urgency, 1));
...
function runSimulation(maxDays, clicks, tierMult, trainMult, farmThreshold, checkpoints) {
  ...
  for (let day = 1; day <= maxDays; day++) {
    ...
    for (let i = 0; i < clicks; i++) {
      runAIWithConfig(ai, clicks, 0, tierMult, trainMult, farmThreshold);
    }
...
const tierMult = 20;  // Fixed at 20x (the winner)
const trainMults = [0.0001, 0.0002, 0.0005, 0.001, 0.002, 0.005, 0.01, 0.02, 0.05, 0.1, 0.2, 0.5, 1, 2, 5, 10, 20];
const farmThresholds = [5, 6, 10];
const clicks = 25;

// Run all simulations
const allResults = {};
for (const ft of farmThresholds) {
  allResults[ft] = {};
  for (const tm of trainMults) {
    const key = `${ft}_${tm}`;
    console.log(`Running FARM_THRESHOLD=${ft}, trainMult=${tm}...`);
    allResults[ft][tm] = runSimulation(500, clicks, tierMult, tm, ft, checkpoints);
  }
}
...
<html>
<head>
  <title>TrainMult × FarmThreshold Comparison (tierMult=20)</title>
  <style>
    body {
...
<body>
  <h1>TrainMult × FarmThreshold Comparison</h1>

  <div class="config">
    <p><strong>Fixed:</strong> tierMult=20, VAL_SL=5, CPS=25</p>
    <p><strong>Testing:</strong> trainMult × FARM_THRESHOLD combinations</p>
  </div>
`;
...
for (const day of checkpoints) {
  ...
  for (const ft of farmThresholds) {
    // Find winner for this threshold
    let maxPower = 0;
    let winnerTM = null;
    for (const tm of trainMults) {
      if (allResults[ft][tm][day].power > maxPower) {
        maxPower = allResults[ft][tm][day].power;
        winnerTM = tm;
      }
    }

    html += `  <h3>FARM_THRESHOLD = ${ft}</h3>
  <table>
    <tr>
      <th>trainMult</th>
      <th>Power</th>
...
`;
    ...
    for (const tm of trainMults) {
      const s = allResults[ft][tm][day];
      const isWinner = tm === winnerTM;
      const isStarving = s.maxStreak >= 3;

      html += `    <tr class="${isWinner ? 'winner' : isStarving ? 'starving' : ''}">
      <td>${tm}</td>
...
// Summary
html += `
  <h2>Power Rankings Summary (Day 500)</h2>
  <div class="summary">
    <table>
      <tr><th>FARM_THRESHOLD</th><th>Best trainMult</th><th>Power</th><th>Starve Days</th></tr>
`;

for (const ft of farmThresholds) {
  const sorted = trainMults
    .map(tm => ({ trainMult: tm, ...allResults[ft][tm][500] }))
    .sort((a, b) => b.power - a.power);
  const best = sorted[0];
  html += `      <tr><td>${ft}</td><td>${best.trainMult}</td><td>${best.power.toExponential(2)}</td><td>${best.starveDays}</td></tr>
`;
}
...
// Write HTML file
fs.writeFileSync('trainmult-threshold-comparison.html', html);
console.log('\nSaved to trainmult-threshold-comparison.html');

// Console summary
console.log(`\n----- POWER RANKINGS (Day 500) -----`);
for (const ft of farmThresholds) {
  const sorted = trainMults
    .map(tm => ({ trainMult: tm, ...allResults[ft][tm][500] }))
    .sort((a, b) => b.power - a.power);
  const best = sorted[0];
  console.log(`FARM_THRESHOLD=${ft}: Best trainMult=${best.trainMult} (${best.power.toExponential(2)}, ${best.starveDays} starve days)`);
}
